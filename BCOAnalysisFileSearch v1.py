...
path = "D:\\Work\\C4C\\NetApp\\For BCO"
...
for line in df['Name']:
    ...
    for name in os.listdir(path):
        valid = False
        if name.endswith(".bo"):
            result = searchterm(txt,os.path.join(path, name))
            if result == True:
                valid = True
                bo.append(name)
                ui.append(' ')
                absl.append(' ')
        elif name.endswith(".uicomponent") or name.endswith(".xuicomponent") or name.endswith(".xml"):
            result = searchterm(txt,os.path.join(path, name))
            if result == True:
                valid = True
                ui.append(name)
...
df = df.transpose()
writer = pd.ExcelWriter('BCO Analysis V2.xlsx', engine='xlsxwriter')  # pylint: disable=abstract-class-instantiated
